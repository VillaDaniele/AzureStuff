$CurrentAzureVMNames = Import-Csv -Path "C:\temp\azurevmnames.csv"
$LatestAzureVMName = Import-Csv -Path "C:\temp\azurevmnames.csv" | Select-Object -Last 1
...
if ($NextVMName -in $CurrentAzureVMNames.VMNAME) {
    ...
    'VMNAME' = $NextVMName
    } | Export-Csv -Path "C:\temp\azurevmnames.csv" -NoTypeInformation -Append -NoClobber
}
